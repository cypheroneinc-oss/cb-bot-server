// /api/answer.js
// *** ランタイム注記：Node.js ランタイムを前提（Edge不可なら注意） ***
...
function toResponsesRow(body = {}) {
  ...
  if (isV2) {
    ...
    if (typeof bodyMeta === 'object') {
      ...
      try { meta = JSON.parse(bodyMeta); } catch (_) { meta = { raw: bodyMeta, parseError: true }; }
    } else {
      meta = {};
    }
  } else {
    meta = { ts: body.ts ?? new Date().toISOString(), ua: null, liffId: null, app: 'c-lab-liff', v: '2025-09' };
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      if (abRows.length) await supabase.from('answers_ab').insert(abRows);

      const mot = (row.answers?.motivation_ordered || []).slice(0,3);
      const motRows = mot.map((m, idx) => ({ response_id: respId, rank: idx+1, motivation: String(m) }));
      if (motRows.length) await supabase.from('motivation_picks').insert(motRows);
    } catch (e2) {
      console.warn('[answer] normalize insert skipped:', e2?.message || e2);
    }

    return res.status(200).json({ ok:true, id: respId });
  } catch (e) {
