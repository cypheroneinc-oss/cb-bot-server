 ⎿ // /api/answer.js
    ...
    function toResponsesRow(body = {}) {
      ...
      if (isV2) {
        ...
        const mot = body.answers?.motivation_ordered ??
    body.answers?.q3 ?? [];
    ...
    [] };
      ...
      const scoring = isV2 ? (body.scoring ??
    computeScoring(answers.ab)) : computeScoring(answers.ab);
    ...
    JSON.parse
      ...
      // responses.result_type は not null
    なので入れる（ここを差し替え）
    ...
    export default async function handler(req, res) {
      ...
      if (req.method !== 'POST') {
        return res.status(405).json({ ok:false, error:'Method Not
    Allowed' });
      }
      ...
      try {
        ...
        if (!row.line_user_id) {
          console.warn('[answer] missing line_user_id. body=',
    req.body);
    ...
    choice: String(v) }));
          ...
          const mot = (row.answers?.motivation_ordered ||
    []).slice(0,3);
          ...
          console.warn('[answer] normalize insert skipped:',
    e2?.message || e2);
        }
        // ------------------------------------------------------------
    ----------
