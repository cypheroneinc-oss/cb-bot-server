...
function syncMotivationHidden() {
  ...
  console.log('[sync] Q3 motivation synchronized:', valsCheckedOrdered('q3'));
}
...
async function initLIFF() {
  try {
    ...
    const dummyProfile = { userId: 'liff-init-failed-' + Date.now(), displayName: 'LIFFエラーユーザー', pictureUrl: null };
...
async function shareOnLINE() {
  ...
  try {
    ...
    try {
      ...
      try { await navigator.clipboard.writeText(text); } catch {}
      alert('共有に失敗しました。トークで本文を貼り付け、画像を添付してください。');
    }
  }
}

// 2) その他アプリ共有（Web Share → フォールバック）
...
document.addEventListener('DOMContentLoaded', () => {
  ['#personalityForm', '#run', '#status', '#progress', '#result'].forEach(selector => {
    const el = $(selector);
    console.log(`DOM check: ${selector} -> ${el ? 'OK' : 'NOT FOUND'}`);
  });
  ...
  if (typeof window.liff !== 'undefined') {
    initLIFF().catch(e => {
      ...
      $('#status').textContent = 'ブラウザモード（LINEアプリでの利用を推奨）';
