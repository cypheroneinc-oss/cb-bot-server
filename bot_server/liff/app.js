 ⎿ /* =========================
    ...
    // ★ あなたの LIFF ID を入れてください（LINE Developers の LIFF
    ID）
    ...
    const $$ = (sel, p = document) =>
    Array.from(p.querySelectorAll(sel));
    ...
    function syncMotivationHidden() {
      ...
      $$('input[name="q3"]', hiddenContainer).forEach(checkbox => {
        ...
        checkbox.removeAttribute('data-order');
      });
     ...
     hiddenContainer);
        if (checkbox) {
          ...
          checkbox.setAttribute('data-order', index + 1);
        }
      });
    }
    ...
    の再同意含む）
      ...
      ['mot1', 'mot2', 'mot3'].forEach(motId => {
        const select = $(`#${motId}`);
        if (select) {
          select.addEventListener('change', syncMotivationHidden);
        }
      });
    ...
    function buildResult(ans) {
      ...
      if (ans.q1 === 'A') sChallenge++; else if (ans.q1 === 'B')
    sPlan++;
      if (ans.q2 === 'A') sChallenge++; else if (ans.q2 === 'B')
    sPlan++;
      ...
      const TYPES = {
        challenge: {
          title: 'チャレンジ先行タイプ💪',
          tagline: '思い立ったらすぐ動ける！まずやってみて、直しながら
    前へ進むのが得意。',
          style:
    'スピード感のある環境／小さく試して改善していく働き方',
          jobs: [
            '企画・プロデュース（新しい案を形にする）',
            'セールス／提案（訪問・オンライン）',
            '広報・SNS運用（発信して人を集める）',
          ...
          advice: '走り出しは強み。あとで「なぜそうしたか」を一言メモに
    残すと説得力UP。小さなゴールを細かく刻むと達成感が積み上がる。'
        },
        plan: {
          title: '計画ていねいタイプ🧭',
          tagline:
    '全体像を整理してから進むほうが力を発揮！再現性や安定感が武器。',
          ...
          advice: '最初に段取りを書き出すと安心感とスピードが両立。区切
    りごとに振り返りをルーチン化すると成果が伸びる。'
        },
        balance: {
          title: 'バランス型🧩',
    ...
    function barnumComments(ans, typeRaw) {
      const out = [];
      const push = (t) => { if (t && !out.includes(t) && out.length <
    3) out.push(t); };
    ...
    '詳細はコンソールを見て'}`);
          throw new Error(`HTTP ${res.status}: ${json?.error ||
    'unknown error'}`);
        } else {
    ...
    // 1) LINE：送信先を選んで「本文＋画像」を送る（アプリ外ならLIFFを
    開き直す）
    ...
    Error('shareTargetPicker unavailable');
     ...
     imgUrl }
        ...
        console.warn('shareTargetPicker error -> fallback to image
    only', e);
        try {
          await liff.shareTargetPicker([{ type: 'image',
    originalContentUrl: imgUrl, previewImageUrl: imgUrl }]);
          try { await navigator.clipboard.writeText(text); } catch {}
          alert('画像だけ送ります。本文はコピー済みなので貼り付けてくだ
    さい。');
        } catch (e2) {
          try { await navigator.clipboard.writeText(text); } catch {}
          alert('共有に失敗しました。トークで本文を貼り付け、画像を添付
    して送ってください。');
        }
      }
    }
    ...
    'C Lab' });
      ...
      alert('画像を開きました。本文はコピー済みです。お好みのアプリで貼
    り付けてください。');
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      // DOM要素の存在確認とログ出力
      const requiredElements = ['#run', '#status', '#progress',
    '#result'];
    ...
    FOUND'}`);
      ...
      initLIFF().catch(e => {
        ...
        $('#status').textContent = 'LINEアプリでの利用を推奨します';
      });
    });
