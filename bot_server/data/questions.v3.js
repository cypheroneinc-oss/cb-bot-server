...
// サーバ側では v3 スコアラーが 1..6 を 0..100 に線形変換し、必要箇所のみ反転します。
...
// --------------------------------------------------
// HEXACO 準拠：H, E(Emotionality), X(Extraversion), A, C, O
// 旧N(Neuroticism)は使用しない。外向性は X を使用。
// reverse は tags.reverse: true で指定。

export default [
  make('Q01', '結果を出しても、自分からアピールしないタイプだ。', { axis: 'Trait', subfactor: 'X', tags: { reverse: true } }), // Extraversion（逆）
  make('Q02', '家族や友人の優しさとか、言葉で涙が出たことがある。', { axis: 'Trait', subfactor: 'A' }),                     // Agreeableness
  make('Q03', '初めての人や場所でも、すぐ馴染める。',                     { axis: 'Trait', subfactor: 'X' }),                     // Extraversion
  ...
  make('Q09', '落ち着いた環境のほうが力を出せる。',                       { axis: 'Trait', subfactor: 'E', tags: { reverse: true } }), // Emotionality（逆）
  make('Q10', '誰かの役に立てると嬉しい。',                               { axis: 'Trait', subfactor: 'A' }),                     // Agreeableness
  make('Q11', '平凡より変化がある方が気分が上がる。',                     { axis: 'Trait', subfactor: 'O' }),                     // Openness
  make('Q12', 'リーダーっぽい立場になると燃える。',                       { axis: 'Trait', subfactor: 'X' }),                     // Extraversion
  ...
  make('Q16', '不安なことがないと、集中力が上がる。',                     { axis: 'Trait', subfactor: 'E', tags: { reverse: true } }), // Emotionality（逆）
  ...
  make('Q25', 'むずかしいことがあっても、前に出て頑張りたい。',             { axis: 'Trait', subfactor: 'X' }),                     // Extraversion
  ...
  make('Q29', 'できれば失敗したくない。',                                 { axis: 'Trait', subfactor: 'E' }),                     // Emotionality（正）
